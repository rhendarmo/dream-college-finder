          ...
          <div className="w-full max-w-xl rounded-[28px] border border-white/40 bg-white/95 p-8 shadow-[0_20px_80px_rgba(15,23,42,0.30)] backdrop-blur md:p-12">
            <div className="text-center">
              ...
              <p className="mt-4 text-lg text-slate-600">
                Join students finding colleges that fit their profile.
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="mt-10 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-4 gap-2 pt-1">
                  <div className={`h-2 rounded-full ${password.length >= 2 ? "bg-blue-500" : "bg-slate-200"}`} />
                  <div className={`h-2 rounded-full ${/[A-Z]/.test(password) ? "bg-blue-500" : "bg-slate-200"}`} />
                  <div className={`h-2 rounded-full ${/\d/.test(password) ? "bg-blue-500" : "bg-slate-200"}`} />
                  <div className={`h-2 rounded-full ${/[^A-Za-z0-9]/.test(password) ? "bg-blue-500" : "bg-slate-200"}`} />
                </div>
              </div>
            ...
            {msg && (
              <div className="mt-6 rounded-2xl border border-green-300 bg-green-50 px-4 py-3 text-green-800">
                {msg}
              </div>
            )}
            ...
            <div className="mt-8 text-center text-xl text-slate-700">
              ...
              <Link className="font-semibold text-blue-700 underline underline-offset-4" href="/login">
                Log In
              </Link>
            </div>
          </div>
